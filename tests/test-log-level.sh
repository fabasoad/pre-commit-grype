...
test_log_level_param_precedence() {
  ...
  output=$(PRE_COMMIT_GRYPE_LOG_LEVEL="${log_level_env_var}" \
    ...
    "--grype-args=--quiet --hook-args=--log-level=${log_level_cmd}" \
    2>&1 >/dev/null)
...
main() {
  ...
  test_log_level_param_precedence "grype-dir" "info" "debug" "false"
  test_log_level_param_precedence "grype-dir" "debug" "info" "true"
  test_log_level_env_var "grype-dir" "debug" "true"
  test_log_level_env_var "grype-dir" "info" "false"
